...
async function run4(inputFile) {
  let input = await getInput4(inputFile);
  console.log("Amount of rolls that can be moved is", processInput4(input));
}
function processInput4(inputA) {
  let input = inputA;
  let canMoveCount = 0;
  const atLimit = 4;
  let spaces = [];
  let removeSomething = false;
  do {
...
function checkForAt(input, row, col) {
  return input && input[row] && input[row][col] && input[row][col] === "@"
    ? 1
    : 0;
}
